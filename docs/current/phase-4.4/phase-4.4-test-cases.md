...
> - ~~`READING_ENABLED`~~ — ⛔ đã xoá (The Reading gỡ bỏ 29/05/2026).
...
> **🛑 The Reading (WOW #1) đã GỠ BỎ (29/05/2026).** Reading v0+v1 phản tác dụng
> (vướng nút demo, chuyển cảnh gập ghềnh, v0 generic-trên-zero-data hại uy tín) —
> xem banner DECISION trong [`phase-4.4-detailed.md`](phase-4.4-detailed.md).
> **TC-006…TC-019, TC-056, TC-057 (và mọi nhắc tới Reading trong các TC khác)
> không còn hiệu lực** — giữ lại chỉ để lưu vết, đánh dấu ~~gạch ngang~~ + ⛔.
> Onboarding nay đi thẳng goal → asset → Twin (`step_2_asset.asset_ack` bắc cầu).
> Hợp đồng gỡ bỏ được khoá bằng pytest `tests/test_phase_4_4/test_onboarding_no_reading.py`
> (không phải manual). Flag `READING_ENABLED` đã bị xoá khỏi codebase.
...
| ~~1~~ | ~~The Reading v0 (đoán mò qua mục tiêu)~~ ⛔ GỠ BỎ | ~~WOW #1~~ |
| 2 | Hỏi tài sản đầu tiên + (tuỳ chọn) chụp màn hình | WOW #2 |
| ~~3~~ | ~~The Reading v1 (đoán trên con số thật)~~ ⛔ GỠ BỎ | — |
| 3 | `asset_ack` ghi nhận số → bắc cầu thẳng sang Twin | — |
| 4 | Twin teaser | — |
...
> ⛔ **Epic 1 (The Reading) đã GỠ BỎ.** TC-006…TC-019 dưới đây không còn chạy —
> bỏ qua khi QA. Thay vào đó, sau khi chọn mục tiêu onboarding đi **thẳng** tới
> bước (2/3) hỏi tài sản (xem TC-005b). Hợp đồng được khoá bằng pytest, không
> manual.
...
- **Mục tiêu:** Salutation đã chọn áp dụng nhất quán ở mọi tin nhắn sau (asset ack, Twin teaser…).
- **Precondition:** Chọn "anh" ở TC-002.
- **Các bước:**
  1. Tiếp tục onboarding tới hết (mục tiêu → tài sản → Twin teaser).
- **Kết quả mong đợi:** Không tin nhắn nào gọi nhầm "chị"/"bạn". Twin teaser gọi đúng "anh".

### TC-005b — Sau khi chọn mục tiêu đi THẲNG tới bước hỏi tài sản (no Reading)
- **Mục tiêu:** Xác nhận The Reading đã gỡ — không còn placeholder "đang đoán…" / lời bói nào chen giữa goal-pick và bước tài sản.
- **Precondition:** Đã chọn xưng hô. Codebase sau 29/05/2026 (không còn `READING_ENABLED`).
- **Các bước:**
  1. Bấm 1 nút mục tiêu (vd "🌱 Hiểu rõ tổng tài sản của tôi").
- **Kết quả mong đợi:** Sau goal-ack hiện **ngay** bước (2/3) hỏi tài sản. KHÔNG có placeholder 🔮, KHÔNG có lời "đoán/bói", KHÔNG độ trễ chờ LLM Reading.

### ~~TC-006 — Reading v0 placeholder hiện ngay sau khi chọn mục tiêu~~ ⛔ GỠ BỎ
...
### ~~TC-007 — Reading v0 nội dung ấm áp, đúng persona~~ ⛔ GỠ BỎ
...
### ~~TC-008 — Reading v0 bám theo mục tiêu đã chọn~~ ⛔ GỠ BỎ
...
### ~~TC-009 — Reading v0 disclaimer mời nhập số thật~~ ⛔ GỠ BỎ
...
### ~~TC-010 — Reading v1 chạy trên con số thật~~ ⛔ GỠ BỎ
...
### ~~TC-011 — Reading v1 phản ánh độ lớn con số~~ ⛔ GỠ BỎ
...
### ~~TC-012 — Reading v0 fallback khi LLM không khả dụng~~ ⛔ GỠ BỎ
...
### ~~TC-013 — Reading v1 fallback khi LLM không khả dụng~~ ⛔ GỠ BỎ
...
### ~~TC-014 — READING_ENABLED 🔴 OFF — bỏ qua The Reading~~ ⛔ GỠ BỎ
> Flag `READING_ENABLED` đã bị xoá; hành vi "không có Reading" nay là **mặc định
> duy nhất** và được TC-005b + pytest bảo vệ. Không còn gì để bật/tắt.

### TC-015 — Persona check: tuyệt đối không có "CFO"
- **Mục tiêu:** Không lộ chữ "CFO"/"Personal CFO" ở bất kỳ tin nhắn user-facing nào.
- **Precondition:** Chạy full onboarding (flag mặc định, không còn Reading).
- **Các bước:**
...
- **Các bước:**
  1. Hoàn tất tới Twin teaser.
- **Kết quả mong đợi:** `asset_ack` + Twin teaser không có giọng chê bai/khuyên dạy gắt. Toàn bộ supportive, đồng hành.

### ~~TC-017 — Xưng hô đúng trong Reading cho cả 3 lựa chọn~~ ⛔ GỠ BỎ
> Xưng hô nay được kiểm ở `asset_ack` + Twin teaser (TC-005). Reading không còn.

### ~~TC-018 — Capitalization đầu câu của salutation~~ ⛔ GỠ BỎ
> Phụ thuộc disclaimer_v0 của Reading — không còn áp dụng.

### ~~TC-019 — Reading không vượt giới hạn độ dài~~ ⛔ GỠ BỎ
...
- **Các bước:**
  1. Gõ "200tr".
- **Kết quả mong đợi:** Bé Tiền nhận = 200,000,000đ, hiện `asset_ack` rồi chuyển sang Twin teaser. Không báo lỗi.
...
- **Kết quả mong đợi:** Hiện ack "Bé Tiền đang đọc ảnh giúp bạn… chờ chút xíu nha 👀", sau đó đọc đúng ~200tr và tiếp tục flow (`asset_ack` → Twin teaser).
...
- **Các bước:**
  1. Bấm nút số đúng.
- **Kết quả mong đợi:** Lưu số đã chọn, hiện `asset_ack` rồi chuyển sang Twin teaser.
...
### TC-041 — Twin teaser xuất hiện ngay sau khi ghi nhận tài sản
- **Mục tiêu:** Phút-4 Twin teaser nối tiếp `asset_ack` (không còn Reading chen giữa).
- **Precondition:** Đã nhập tài sản thật.
- **Các bước:**
  1. Hoàn tất nhập tài sản → đọc `asset_ack`.
- **Kết quả mong đợi:** Ngay sau `asset_ack` ("✅ Bé Tiền ghi nhận: <số>… Để Bé Tiền vẽ Twin tài chính cho <tên> nhé…"), Bé Tiền hiện Twin teaser rồi tới bước (3/3) "Twin của bạn". KHÔNG có lời đoán/bói xen giữa.
...
- **Mục tiêu:** Toàn bộ flow diễn ra đúng thứ tự với tất cả flag mặc định.
- **Precondition:** Flags default (Screenshot OFF, Proactive ON; không còn Reading). User mới.
- **Các bước:**
  1. Welcome → chọn mục tiêu → nhập tên → chọn xưng hô → nhập tài sản → `asset_ack` → Twin teaser → Twin shown.
- **Kết quả mong đợi:** Thứ tự đúng bảng choreography (không có bước Reading), không bước nào bị nhảy/lặp, xưng hô nhất quán, không lỗi kỹ thuật, không "CFO".

### TC-050 — Tất cả flag OFF — flow tối giản vẫn hoàn tất
- **Mục tiêu:** Tắt cả flag còn lại, onboarding vẫn đi tới Twin.
- **Precondition:** Screenshot 🔴, Proactive 🔴 (Reading đã gỡ vĩnh viễn). User mới.
- **Các bước:**
  1. Chạy onboarding tới hết.
- **Kết quả mong đợi:** Không hint chụp ảnh, không nhắn chủ động. Vẫn: welcome → mục tiêu → tên → xưng hô → nhập tài sản → `asset_ack` → Twin. Hoàn tất bình thường.

### TC-051 — Các flag còn lại ON (gồm screenshot) — full WOW
- **Mục tiêu:** Bật hết flag còn lại, mọi WOW còn hiệu lực đều xuất hiện.
- **Precondition:** Screenshot 🟢, Proactive 🟢. User mới.
- **Các bước:**
  1. Onboarding dùng ảnh số dư để nhập tài sản, rồi im lặng.
- **Kết quả mong đợi:** WOW #0 (xưng hô), #2 (đọc ảnh), `asset_ack` → Twin, #3 (nhắn chủ động) đều hoạt động đúng. KHÔNG có WOW #1 / Reading.
...
- **Kết quả mong đợi:** Hệ thống xử lý nhất quán (chốt mục tiêu cuối hoặc khoá sau lần chọn đầu), đi thẳng tới bước hỏi tài sản, không trạng thái rối.

### ~~TC-056 — Gửi tin text giữa lúc Reading đang chạy~~ ⛔ GỠ BỎ
> Không còn placeholder/độ trễ Reading để chen vào — không áp dụng.

### ~~TC-057 — LLM trả về nội dung lỗi định dạng (parse fail) → fallback~~ ⛔ GỠ BỎ
> Reading là LLM call duy nhất ở phút-1; đã gỡ nên không còn fallback Reading.
...
- **Mục tiêu:** Mọi copy đến từ YAML, không lộ `{amount}`, `{name}`, `{warning}` chưa điền (vd trong `asset_ack`, screenshot quality warning).
- **Precondition:** Chạy full onboarding (gồm screenshot quality warning).
- **Các bước:**
...
- **Mục tiêu:** Với Proactive OFF + screenshot OFF, trải nghiệm tương đương trước 4.4.
- **Precondition:** Proactive 🔴, Screenshot 🔴 (Reading đã gỡ vĩnh viễn).
- **Các bước:**
  1. Onboarding đầy đủ.
...
|---|---|
| Epic 0 — Salutation | TC-001…TC-005, TC-045, TC-060 |
| ~~Epic 1 — The Reading~~ ⛔ GỠ BỎ | ~~TC-006…TC-019, TC-056, TC-057~~ → thay bằng TC-005b (đi thẳng goal→asset) + pytest `test_onboarding_no_reading.py` |
...
| Feature flags on/off | TC-029/030, TC-040, TC-043/044, TC-050, TC-051 |
| Persona / no-CFO / no-judgment | TC-015, TC-016, TC-048 |
| Welcome / source variants | TC-052, TC-053 |
| Validation / edge / regression | TC-024…TC-026, TC-054, TC-055, TC-058, TC-059, TC-060 |

**Definition of Done (manual QA):** Tất cả TC còn hiệu lực PASS với flag mặc định
(Screenshot OFF, Proactive ON; Reading đã gỡ vĩnh viễn) + xác minh riêng các tổ
hợp flag ở Batch 3. Hợp đồng "không còn Reading" do pytest
`tests/test_phase_4_4/test_onboarding_no_reading.py` bảo vệ (không manual).
