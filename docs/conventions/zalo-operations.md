...
| Image caption length | **300 chars** — the same ceiling as any text message, because `send_image_message` puts the caption in the top-level `message.text` alongside the attachment | `ASSUMED` (5.1 #2.4) | Send one image with a ~299-char caption to a staging OA. If it is rejected or visibly clipped, the caption has its own smaller budget: lower the `truncate_for_zalo` limit at the `send_photo` call site only, not `ZALO_MESSAGE_MAX_CHARS`. Getting this wrong the other way (assuming 100) split every Twin into an image plus a follow-up text. |
