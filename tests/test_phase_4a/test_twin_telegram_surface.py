...
async def test_twin_handler_empty_state_uses_notifier(monkeypatch):
    ...
    monkeypatch.setattr(twin_handler.twin_query_service, "get_twin_snapshot", fake_snapshot)
    notifier = FakeNotifier()

    await twin_handler.send_twin_current(object(), chat_id=123, user=FakeUser(), notifier=notifier)
...
async def test_twin_handler_sends_photo_with_cone_caption(monkeypatch):
    ...
    monkeypatch.setattr(twin_handler.twin_query_service, "get_twin_snapshot", fake_snapshot)
    monkeypatch.setattr(twin_handler.twin_query_service, "get_latest_projection", fake_latest)
    monkeypatch.setattr(twin_handler, "render_projection_chart", lambda cone, optimal=None: b"png-bytes")
